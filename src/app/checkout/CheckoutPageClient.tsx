      ...
      <div className="space-y-5">
        ...
        <Section title="Data Akun">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Player ID" placeholder="contoh: 123456789(1234)" value={playerId} onChange={(e) => setPlayerId(e.target.value)} />
            <Field label="Server/Region (opsional)" placeholder="contoh: Asia / 2150" value={server} onChange={(e) => setServer(e.target.value)} />
            <Field label="Kontak (opsional)" placeholder="Email / WhatsApp untuk bukti bayar" value={contact} onChange={(e) => setContact(e.target.value)} className="sm:col-span-2" />
          </div>
        </Section>

        <Section title="Metode Pembayaran">
          <div className="grid gap-3 sm:grid-cols-3">
            <MethodCard active={payment === "qris"} icon={QrCode} title="QRIS" desc="Scan semua e-wallet/bank" onClick={() => setPayment("qris")} />
            <MethodCard active={payment === "ewallet"} icon={Wallet2} title="E-Wallet" desc="OVO / Dana / GoPay / ShopeePay" onClick={() => setPayment("ewallet")} />
            <MethodCard active={payment === "va"} icon={CreditCard} title="Virtual Account" desc="BCA / BNI / Mandiri / dll" onClick={() => setPayment("va")} />
          </div>
        </Section>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="inline-flex items-center gap-2 text-xs text-white/50">
            <ShieldCheck className="h-4 w-4" /> Pembayaran aman. Otomatis & bisa refund jika gagal.
          </div>
          <button
            onClick={submit}
          ...
          >
            {status === "loading" ? (<><Loader2 className="h-4 w-4 animate-spin" /> Memproses…</>) : (<><CreditCard className="h-4 w-4" /> Bayar Sekarang</>)}
          </button>
        </div>
      </div>
      ...
      <aside className="space-y-5">
        <Section title="Ringkasan Pembayaran">
          <div className="space-y-2 text-sm">
            <div className="flex items-center justify-between"><span>Harga</span><span>{item ? currency(item.price) : "-"}</span></div>
            <div className="flex items-center justify-between"><span>Biaya {payment.toUpperCase()}</span><span>{item ? currency(fee) : "-"}</span></div>
            <div className="my-2 border-t border-white/10" />
            <div className="flex items-center justify-between text-base font-semibold"><span>Total</span><span>{item ? currency(item.price + fee) : "-"}</span></div>
          </div>
        </Section>
